...
#   6. Font fix (missing/blank characters)
#   7. Space optimization (bind mounts)
#   8. Browser & system performance optimizations
#
# Prerequisites:
...
echo "[1/9] Remounting root filesystem read-write..."
mount -o remount,rw / || true

# Install performance tuning service
echo "[2/9] Installing performance tuning service..."
...
echo "[3/9] Installing zRAM resize service (1.5GB)..."
...
echo "[4/9] Installing PSI-based OOM guard..."
...
echo "[5/9] Installing framebuffer boot status display..."
...
echo "[6/9] Installing PulseAudio audio crackling fix..."
...
echo "[7/9] Fixing fonts (missing/blank characters)..."
sh "${SCRIPT_DIR}/fix_fonts.sh"
echo "  Done."

# Create bind mount directories
echo "[8/9] Setting up space optimization (bind mounts)..."
mkdir -p /home/.system/apt-cache /home/.system/log
echo "  Done."

# Browser & system performance optimizations
echo "[9/9] Applying browser & system performance optimizations..."
sh "${SCRIPT_DIR}/optimize_performance.sh"
echo "  Done."
...
systemctl is-enabled browser-optimizations.service 2>/dev/null && echo "  [x] browser-optimizations.service"
systemctl is-enabled browser-cache-tmpfs.service 2>/dev/null && echo "  [x] browser-cache-tmpfs.service"
systemctl is-enabled tune-priorities.timer 2>/dev/null && echo "  [x] tune-priorities.timer"
systemctl is-enabled disable-fbcon.service 2>/dev/null && echo "  [x] disable-fbcon.service"
echo ""
